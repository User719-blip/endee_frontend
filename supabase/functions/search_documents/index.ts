...
Deno.serve(async (req) => {
  try {
    ...
    const { vector, top_k = 3 } = await req.json();
    ...
    const response = await requestEndee(`/v1/collections/${COLLECTION_NAME}/search`, {
      vector,
      top_k,
    });
    ...
    const mapped = Array.isArray(payload)
      ? payload.map((item: any) => ({
          ...
          content: item.payload?.text ?? item.payload?.content ?? '',
    ...
    return new Response(
      JSON.stringify({
        error: 'search_documents function crashed',
